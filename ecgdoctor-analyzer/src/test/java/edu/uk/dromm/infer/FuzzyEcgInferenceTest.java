...
public class FuzzyEcgInferenceTest {
	...
	private void assertInputRanges(final FunctionBlock fb, final double rrexp,
			final double qrsexp, final double pvexp, final double prexp) {
		final double rrmemb = fb.getVariable("rr").getMembership("normal");
		Assert.assertEquals("RR membership should be near " + rrexp, rrexp,
				rrmemb, 0.01);
		final double pvmemb = fb.getVariable("pv").getMembership("present");
		Assert.assertEquals("PV membership should be near " + pvexp, pvexp,
				pvmemb, 0.01);
		final double qrsmemb = fb.getVariable("qrs").getMembership("normal");
		Assert.assertEquals("QRS membership should be near " + qrsexp, qrsexp,
				qrsmemb, 0.01);
		final double prmemb = fb.getVariable("pr").getMembership("normal");
		Assert.assertEquals("PR membership should be near " + prexp, prexp,
				prmemb, 0.01);
	}
