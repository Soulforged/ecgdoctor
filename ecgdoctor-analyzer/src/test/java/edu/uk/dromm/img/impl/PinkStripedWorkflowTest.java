...
public class PinkStripedWorkflowTest {
	...
	public void variants() {
		final URL[] resources = new URL[] {
				...
				this.getClass().getResource(
						"/image/ecg-pink-M224Mo249Std39Sk-2Ku7.jpg"),
				this.getClass().getResource(
						"/image/ecg-pink-M201Mo234Std41Sk-1Ku0.jpg") };
			...
			try {
				...
				ImageIO.write(
						...
						new FileOutputStream("target/"
								+ splitted[splitted.length - 1].replaceAll(
										"\\..*", "-out.png")));
				Assert.assertTrue(String.format(formatMessage, "Mean", url,
						">= 250", is.mean), is.mean >= 250);
				Assert.assertTrue(String.format(formatMessage, "Mode", url,
						">= 255", is.mode), is.mode >= 255);
				Assert.assertTrue(String.format(formatMessage,
						"Standard deviation", url, ">= 13", is.stdDev),
						is.stdDev >= 13);
