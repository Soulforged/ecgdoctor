...
public class ECGImageAnalisysTest {
	...
	public void process() {
		...
		final ImageProcessor ip = new BinaryProcessor(new ByteProcessor(
				resultantBi));
	...
	public void obtainingZeroesLineMethod() {
		...
		for (int i = 0; i < ip.getHeight(); i++) {
			...
			if (allPointsCopy.size() > ip.getHeight() * 0.1) {
				List<Integer> highstList = highestPointCount.get(allPointsCopy
						.size());
				if (highstList == null) {
		...
		CollectionUtils.filter(firstPoints,
				new ByYSeparation(firstPoints.get(0), maxY));
		final List<Integer> secondPoints = new ArrayList<Integer>(
				sortableIntersection);
		CollectionUtils.filter(
				secondPoints,
				new ByYSeparation(secondPoints.get(firstPoints.get(firstPoints
						.size() - 1)), maxY));
		final List<Integer> thirdPoints = new ArrayList<Integer>(
				sortableIntersection);
		CollectionUtils.filter(
				firstPoints,
				new ByYSeparation(thirdPoints.get(secondPoints.get(secondPoints
						.size() - 1)), maxY));
