...
public class DefaultECGImageAnalisys implements ImageAnalysis {
    ...
    public ECGParameters process(final BufferedImage bi) {
        ...
        final double avgLeadHeight = 0.05 * ip.getHeight();
        ...
        final List<HeightLengths> allLenghtsPerHeight = mapLengths(
                allPointsPerHeight);
        final List<Range> relevantRanges = heightRanges(heightsOrdered, avgLeadHeight);
    ...
    private static Point detectStart(final List<Point> coalescedPoints) {
        System.out.println(coalescedPoints);
        int zCount = 0;
        Point sp = null;
     ...
     * aproximated margin.
     *
     * @param differentHeightsOrdered - the heights of a vertical section, calculated by calling
     *                                {@link #differentHeights(List)} and passing
     *                                {@link #allPoints(List, int, int)} into it.
     * @param margin                  - an arbitrary range. If
    ...
    private static List<Range> heightRanges(
            final Collection<Integer> differentHeightsOrdered, final double margin) {
     ...
     * </p>
     *
     * @param lowerMargin - the lower margin of a point section, usually the lesser value of
     *                    Y belonging to a lead.
     * @param upperMargin - the higher margin of a point section, usually the higher value
     *                    of Y belonging to a lead.
     * @param lengths     - all the lengths collected for each height
     * @return
     */
     ...
     *
     * @param allPoints - all the ECG mapped points usually obtained calling
     *                  {@link #allPoints(ImageProcessor)}
     * @param section   - the number of the vertical section to collect, valid [1-4]
     * @param width     - the width of the image that's actually filled by the ECG
     ...
     *
     * @param relevantRanges             - all the ranges considered relevant, that's the ranges for each
     *                                   significant height
     * @param lead                       - the lead number, from 0 to 3, this actually identifies the
     *                                   vertical position of a lead
     * @param allPointsInVerticalSection - all the points in a vertical section of the ECG
     * @return
     */
     ...
     *
     * @param pointsInLead - points in lead to calibrate
     * @param zero         - the zero's height of the lead
     * @return
     */
